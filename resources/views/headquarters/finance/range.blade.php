...
@section('content')
    {!! Form::open(['method'=>'POST','action'=>'National\NationalFinanceController@rangepost','class'=>'modal form-row','id'=>'date', 'tabindex'=>'-1','aria-hidden'=>'true','role'=>'dialog'] ) !!}
                ...
                @if($regions)
                    <table class="table table-striped table-hover  table-scroll-vertical">
                        ...
                        @foreach($regions as $region)
                            <tr>
                                <td><a class="btn-link" href="{{route('nationalRangeArea',$region->id)}}">{{$region->name}}</a></td>
                                <td>
                                    <?php
