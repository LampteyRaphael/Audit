...
@section('dashboard')
    <li>
        <p class="navbar-text">
            MONTHLY TITHE RECORDS
        </p>
    </li>
@endsection

@section('content')
    {!! Form::open(['method'=>'POST','action'=>'National\NationalFinanceController@monthlypost','class'=>'modal form-row','id'=>'date', 'tabindex'=>'-1','aria-hidden'=>'true','role'=>'dialog'] ) !!}
                ...
                @if($regions)
                    <table class="table table-striped table-hover  table-scroll-vertical">
                        ...
                        @foreach($regions as $region)
                            ...
                            <tr>
                                <td><a class="btn-link" href="{{route('nationalMonthArea',$region->id)}}">{{$region->name}}</a></td>
                                <td>
                                    <?php
