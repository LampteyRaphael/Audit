...
@section('dashboard')
    ...
    <li>
        {!! Form::open(['method'=>'POST', 'action'=>'National\NationalLocalSearchController@empty2','onsubmit' => 'return ConfirmDelete()']) !!}
        <p class="navbar-text">
            {!! Form::submit('Empty The Recycle bin',['class'=>'btn btn-danger btn-xs']) !!}
            {!! Form::close() !!}
        </p>
    </li>
@endsection
                        ...
                        @if($users)
                            @foreach($users as $user)
                                <tr>
                                    ...
                                    <td>{{$user->local? $user->local->name:''}}</td>
