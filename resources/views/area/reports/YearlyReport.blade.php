...
@section('content')
    ...
    {!! Form::open(['method'=>'POST','action'=>'Area\FinanceDailyController@store3','class'=>'modal form-inline','id'=>'date', 'tabindex'=>'-1','aria-hidden'=>'true','role'=>'dialog'] ) !!}
                ...
                <div class="form-group">
                    <label for="date" class="control-label">
                        <div class="input-group">
                            <div class="input-group-addon bg-blue"><span class="bold">Change Date</span></div>
                        ...
                        @if($incomeCategory)
                            <table class="table table-striped">
                                ...
                                @foreach($incomeCategory as $item)
                                    <?php
                                    $sumIncome=App\AreaIncome::where("area_id",Auth::user()->area_id)->where('area_income_categories_id',$item->id)
                        ...
                        @if($incomeCategoryE)
                            <table class="table table-striped">
                                ...
                                @foreach($incomeCategoryE as $item)
                                    <?php $sumExpenditure=App\AreaExpenditure::where("area_id",Auth::user()->area_id)->where('area_income_categories_id',$item->id)
                                        ->whereYear('created_at',$year)
                                        ->pluck('amount')->sum(); ?>
